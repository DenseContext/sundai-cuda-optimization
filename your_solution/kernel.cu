...
static RepCache s_act_cache, s_wgt_cache, s_sa_cache;
    ...
    int M, int N, int nkt, int num_groups_B)
{
    ...
    // K-tile loop (single B-group since weight_group_size = K)
    for (int kt = 0; kt < nkt; kt++) {

        // A scales: coalesced read from repacked [M/32, nkt, 32] layout
        const half scale_lane = scales_A[((bm * NW4 + warp) * nkt + kt) * WM4 + lane];
        ...
        for (int nt = 0; nt < NT; nt++) {
            ...
            acc[0][nt][0] = __hfma2(__floats2half2_rn((float)p0[0], (float)p0[1]), sa0, acc[0][nt][0]);
            acc[0][nt][1] = __hfma2(__floats2half2_rn((float)p0[2], (float)p0[3]), sa1, acc[0][nt][1]);
            acc[0][nt][2] = __hfma2(__floats2half2_rn((float)p1[0], (float)p1[1]), sa0, acc[0][nt][2]);
            acc[0][nt][3] = __hfma2(__floats2half2_rn((float)p1[2], (float)p1[3]), sa1, acc[0][nt][3]);

            acc[1][nt][0] = __hfma2(__floats2half2_rn((float)q0[0], (float)q0[1]), sa2, acc[1][nt][0]);
            acc[1][nt][1] = __hfma2(__floats2half2_rn((float)q0[2], (float)q0[3]), sa3, acc[1][nt][1]);
            acc[1][nt][2] = __hfma2(__floats2half2_rn((float)q1[0], (float)q1[1]), sa2, acc[1][nt][2]);
            acc[1][nt][3] = __hfma2(__floats2half2_rn((float)q1[2], (float)q1[3]), sa3, acc[1][nt][3]);
        }
    ...
    // Epilogue: apply B-scale (deferred from inner loop) and store
    ...
    for (int nt = 0; nt < NT; nt++) {
        ...
        *reinterpret_cast<half2*>(&C[m1 * N + c0]) = __hmul2(acc[0][nt][1], sb01);
        *reinterpret_cast<half2*>(&C[m1 * N + c2]) = __hmul2(acc[0][nt][3], sb23);

        *reinterpret_cast<half2*>(&C[m2 * N + c0]) = __hmul2(acc[1][nt][0], sb01);
        *reinterpret_cast<half2*>(&C[m2 * N + c2]) = __hmul2(acc[1][nt][2], sb23);
        *reinterpret_cast<half2*>(&C[m3 * N + c0]) = __hmul2(acc[1][nt][1], sb01);
        *reinterpret_cast<half2*>(&C[m3 * N + c2]) = __hmul2(acc[1][nt][3], sb23);
    }
}
    ...
    int grid_n, int grid_m)
{
    ...
    // K-tile loop (B-scale deferred to epilogue)
    for (int kt = 0; kt < nkt; kt++) {

        // A scales: coalesced read from repacked [M/32, nkt, 32] layout
        const half scale_lane = scales_A[((bm * NW + warp) * nkt + kt) * WM + lane];
        const half sa0_s = __shfl_sync(0xffffffff, scale_lane, row);
        const half sa1_s = __shfl_sync(0xffffffff, scale_lane, row + 8);
        const half sa2_s = __shfl_sync(0xffffffff, scale_lane, row + 16);
        const half sa3_s = __shfl_sync(0xffffffff, scale_lane, row + 24);
        ...
        // N-tile compute loop — B-scale deferred to epilogue
        #pragma unroll
        for (int nt = 0; nt < NT; nt++) {
            uint4 bf = B[b_base + (size_t)kt * b_stride + nt * WS];
    ...
    torch::Tensor scales_A, torch::Tensor scales_B, int group_size)
{
    ...
    // Cache repacked A-scales: [M, nkt] → [M/32, nkt, 32] for coalesced access
    uintptr_t sa_key = tkey(scales_A);
    if (!s_sa_cache.ok || s_sa_cache.k1 != sa_key) {
        s_sa_cache.data = scales_A.reshape({M / WM, WM, nkt}).transpose(1, 2).contiguous();
        s_sa_cache.k1 = sa_key; s_sa_cache.ok = true;
    }
    const half* sa_ptr = reinterpret_cast<const half*>(s_sa_cache.data.data_ptr<at::Half>());
